...
for(i in 1:n){
  ...
  # Dopasowanie modelu CTree do danych treningowych
  model_ctree <- ctree(output ~ ., train_data)
  predictions <- predict(model_ctree, test_data)
  predictions_binary <- ifelse(predictions > 0.5, 1, 0)
  confusion_matrix <- table(test_data$output, predictions_binary)
